...
@app.route('/busLogic',methods=['POST'])
def busLogic():
    ...
    print str(stop1)
    #x =  Process(target=dm.query, kwargs=(stop_a=str(stop1),stop_b=str(stop2),route=int(busNumber))).start()
    x = dm.query(stop_a=str(stop1),stop_b=str(stop2),route=int(busNumber))
